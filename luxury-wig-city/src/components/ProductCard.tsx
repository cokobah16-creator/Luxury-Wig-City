...
import { formatNaira } from '../data/wigs'
 ...
 */
export const ProductCard: React.FC<ProductCardProps> = ({ wig, index = 0 }) => {
  const badgeColor = {
    'Bestseller':       'bg-gold text-burgundy',
    'New':              'bg-burgundy text-gold',
    'Limited':          'bg-black text-gold',
    'Editor\'s Pick':   'bg-gold text-burgundy'
  }[wig.badge || 'Bestseller']

  return (
    <Link to={`/shop/${wig.id}`} className="group lift block">
      <div className="relative aspect-[3/4] bg-burgundy rounded-sm overflow-hidden">
        ...
        {/* Stylized wig silhouette */}
        <svg viewBox="0 0 200 270" className="absolute inset-0 w-full h-full opacity-90" preserveAspectRatio="xMidYMid slice">
          <path
            d={
              wig.texture === 'Curly' ? 'M 40 130 Q 35 60 100 40 Q 165 60 160 130 Q 175 220 150 265 L 50 265 Q 25 220 40 130 Z'
              : wig.texture === 'Wavy' ? 'M 50 125 Q 45 55 100 40 Q 155 55 150 125 Q 165 215 145 265 L 55 265 Q 35 215 50 125 Z'
              : wig.texture === 'Kinky' ? 'M 35 115 Q 30 50 100 35 Q 170 50 165 115 Q 180 215 155 265 L 45 265 Q 20 215 35 115 Z'
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-burgundy-900 via-burgundy-900/80 to-transparent">
          <div className="text-[10px] tracking-[0.18em] uppercase text-gold/80 mb-1">{wig.category}</div>
          <div className="font-serif text-offwhite text-xl font-semibold leading-tight">{wig.name}</div>
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-baseline gap-2">
              <span className="text-gold font-bold text-sm">{formatNaira(wig.price)}</span>
              {wig.oldPrice && (
                <span className="text-offwhite/50 text-xs line-through">{formatNaira(wig.oldPrice)}</span>
              )}
            </div>
            <div className="text-offwhite/70 text-xs">★ {wig.rating}</div>
          </div>
        </div>
